...
update youtube_channels
 ...
 where vtuber_id = 'miru';

insert into youtube_channel_view_statistic (vtuber_id, time, value)
     values ('miru', :today - interval '32 day', 100),
            ('miru', :today - interval '8 day', 400),
            ('miru', :today - interval '2 day', 800);

do $$
declare
    youtube_channel youtube_channels%rowtype;
begin
    select * from youtube_channels into youtube_channel where vtuber_id = 'miru';

    assert(youtube_channel.vtuber_id = 'miru');
...
insert into youtube_streams (stream_id, title, vtuber_id, updated_at, status)
     values ('poi', 'Stream Title', 'miru', :today, 'ended');
